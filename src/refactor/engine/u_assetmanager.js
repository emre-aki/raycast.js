(function ()
{
    ...
    {
        const transposed = new Uint8ClampedArray((w * h) << 2);
        const skipWrRow = (h - 1) << 2;
        let rdoff = 0, wroff = 0;
        for (let y = 0; y < h; ++y)
        {
            wroff = y << 2;
    ...
    function
    U_LoadTextures
    ( textures, verticalFaceWidths, worldHeights,
      frames, fps )
    {
    ...
    function
    U_LoadSingleSprite
    ( sprites, ids, idx, length, frames, fps,
      resolve, reject )
    {
